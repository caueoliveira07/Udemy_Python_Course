...
entrada = input('Digite seu CPF [000.000.000-00]: ')
...
for numero in cpf[:9]:
    primeiro_digito += int(numero) * multiplicador
    multiplicador -= 1
...
for numero in cpf[:9] + str(primeiro_digito):
    segundo_digito += int(numero) * multiplicador
    multiplicador -= 1

segundo_digito = (segundo_digito * 10) % 11
segundo_digito = 0 if segundo_digito > 9 else segundo_digito

cpf_gerado_pelo_calculo = f'{cpf[:9]}{primeiro_digito}{segundo_digito}'
